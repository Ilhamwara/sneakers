...
elseif ($page == 'form-pemesanan') {
	...
	if (isset($_GET['key'])) {
		...
		$id = $_GET['key'];
		$query = mysql_query("SELECT pemesanan.id,member.nama,barang.foto,barang.nama_barang,pemesanan.status,pemesanan.tanggal,pemesanan.bukti,pemesanan.harga,pemesanan.qty FROM pemesanan INNER JOIN barang ON pemesanan.id_barang=barang.id INNER JOIN member ON pemesanan.id_user=member.id WHERE pemesanan.id='$id'");
